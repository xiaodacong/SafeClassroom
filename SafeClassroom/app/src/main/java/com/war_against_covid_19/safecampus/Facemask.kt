...
import com.war_against_covid_19.safecampus.ml.FackMaskDetection
...
class Facemask : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnQr.setOnClickListener {
            var i : Intent = Intent(this@Facemask, QrCode::class.java)
            //i.putExtra("address",)
            startActivity(i)
        }
        btnEndSession.setOnClickListener {
            var i : Intent = Intent(this@Facemask, FunctionChoose::class.java)
            //i.putExtra("address",)
            startActivity(i)
        }
        btnCameraLensFace.isVisible=false
        ...
        lifecycleScope.launch(Dispatchers.Main) {
            if (output.isNotEmpty()) {
                ...
                output.firstOrNull()?.let {
                        ...
                        category ->
                  /*  if ((category != null) and (category.label != "without_mask")) {
                        var i: Intent = Intent(this@Facemask, QrCode::class.java)
                        //i.putExtra("address",)
                        startActivity(i)
                    }
                    */
                    btnQr.isVisible = category.label != "without_mask"
                    ...
                    txtOutput.setTextColor(
                        ContextCompat.getColor(
                            applicationContext,
                            if (category.label == "without_mask") color.holo_red_dark else color.holo_green_light
                        )
                    )
                    overlay.background =
                        getDrawable(if (category.label == "without_mask") R.drawable.red_border else R.drawable.green_border)
